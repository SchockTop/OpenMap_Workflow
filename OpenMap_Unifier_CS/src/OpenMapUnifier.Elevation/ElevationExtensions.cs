...
namespace OpenMapUnifier.Elevation;
...
public static class ElevationExtensions
{
        ...
        GeoPoint geo, ICoordinateTransform? transform = null, CancellationToken ct = default)
    {
        ...
        return provider.GetElevationAsync(transform.ToUtm(geo), ct);
    }
    ...
    /// </summary>
    public static async Task<IReadOnlyList<(UtmPoint Position, double? Elevation)>> GetProfileAsync(
        this IElevationProvider provider, UtmPoint from, UtmPoint to, int samples = 100,
        CancellationToken ct = default)
    {
        if (samples < 2) throw new ArgumentOutOfRangeException(nameof(samples), "Need at least 2 samples.");
        var result = new (UtmPoint, double?)[samples];
    ...
    public static async Task<(double SlopeDegrees, double AspectDegrees)?> GetSlopeAspectAsync(
        this IElevationProvider provider, UtmPoint p, double stepMeters = 1.0,
        CancellationToken ct = default)
    {
        var east = await provider.GetElevationAsync(new UtmPoint(p.Easting + stepMeters, p.Northing), ct).ConfigureAwait(false);
        var west = await provider.GetElevationAsync(new UtmPoint(p.Easting - stepMeters, p.Northing), ct).ConfigureAwait(false);
        var north = await provider.GetElevationAsync(new UtmPoint(p.Easting, p.Northing + stepMeters), ct).ConfigureAwait(false);
        var south = await provider.GetElevationAsync(new UtmPoint(p.Easting, p.Northing - stepMeters), ct).ConfigureAwait(false);
